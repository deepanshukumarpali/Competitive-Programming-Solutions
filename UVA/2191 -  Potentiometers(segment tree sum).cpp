...
ll Construct(ll l=0,ll r=n-1,ll cur=0)
{
    if(l==r)
    {
        st[cur]=a[l];
    ...
    ll m=l+(r-l)/2;
    st[cur]=Construct(l,m,2*cur+1)+Construct(m+1,r,2*cur+2);
    return st[cur];
}
...
void Update(ll ind,ll extra,ll l=0,ll r=n-1,ll cur=0)
{
    ...
    ll m=l+(r-l)/2;
    Update(ind,extra,l,m,2*cur+1);
    Update(ind,extra,m+1,r,2*cur+2);
